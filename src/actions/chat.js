...
export function sendMessage(message) {
  return (dispatch, getState) => {
    ...
    dispatch({
      type: actTypes.SEND_MESSAGE_REQUEST,
    });

    return makeRequest({
      endpoint: `/chats/${activeId}`,
      token,
      body: {
        data: {
          content: message,
      ...
      .then(data => {
        dispatch({
          type: actTypes.SEND_MESSAGE_SUCCESS,
          payload: data,
        });

        return dispatch(getChat(activeId));
      })
      .then(data => {
        dispatch({
          type: actTypes.MESSAGES_UPDATED,
          payload: data,
        });
      })
      .catch(error => dispatch({
        type: actTypes.SEND_MESSAGE_FAILURE,
        payload: error,
      }));
  }
}
